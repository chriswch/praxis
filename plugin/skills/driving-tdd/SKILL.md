...
- **Session summary** (medium+ tasks) — design decisions and notes for downstream stages. Its first line is a machine-readable status an orchestrator branches on (`craft` consumes it — see `craft/references/contracts.md`): `Status: complete`, `Status: needs-design: <reason>` (a design call must be settled before the tests encode it — routes to `sketching-design`, then re-enters here; raised either up front when placement is non-obvious (step 1 Case B) or mid-loop when refactor exposes a wrong data shape (step 5)), or `Status: blocked: <reason>`.
...
5. **Refactor: let design emerge.**
   ...
   - **A wrong data shape is the one exception to "keep refactor local."** Where the code is telling you a different shape would delete a whole family of branches — an optional field that is never actually absent, a type that permits a state the domain forbids, a payload the callers all have to re-derive — that is not a local tidy and it is not advisory. Stop and emit `Status: needs-design`. The window is now: only this slice's tests encode the shape, so changing it is cheap; after review it means rewriting tests that already passed, which is the expensive path this stage exists to avoid.
...
7. **Feedback loop.**
   ...
   - Data-shape divergence noticed during refactor (step 5) → not advisory. Emit `Status: needs-design: <which shape is wrong and what the code showed>` and stop, so the shape is re-decided while only this slice's tests depend on it.

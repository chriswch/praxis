...
**Domain note**: the four-axis examples and the `fit` score in every reference file are SaaS/software-flavored — `fit` is a *SaaS-relevance* heuristic, not a measure of a method's general quality (a method can be low-`fit` for software yet central to a clinical, policy, or personal decision). For a decision outside software/SaaS, the process and the domain-neutral categories (Minerva, A, B, C, D, E) transfer directly — each of those files now carries a **"Beyond software"** worked example on its most universal methods (a clinical, policy, hiring, or personal-finance case) to model the translation; do the same for the four-axis examples in the decision's own domain.
...
| `a-discipline-and-process.md` | A. Decision science discipline & process | 16 | Decision Intelligence, Value of Information / EVPI, decision quality vs outcome, Decision Quality six-link chain (Spetzler), expected value & decision trees, certain equivalent & risk tolerance, tornado / sensitivity, influence diagrams |
...
| `c-decision-process-and-governance.md` | C. Decision process & governance | 20 | Type-1/Type-2 doors, RAPID/RACI/DACI, premortem, decision records (ADR), WRAP, OODA, AHP, Kepner-Tregoe, kill criteria & quit review, Toyota Kata, PrOACT / even-swaps, consent / advice process / 6-pager |
...
| `e-biases-and-debiasing.md` | E. Cognitive biases & debiasing | 21 | Confirmation bias, anchoring, sunk cost, framing effect, status quo bias, red team, reference-class forecasting, calibration & proper scoring (Brier), forecast aggregation & extremizing |
...
| `h-experimentation-and-causal.md` | H. Experimentation & causal inference | 18 | A/B testing, guardrail metrics, DiD, holdouts, CUPED, sample-size/MDE, causal diagrams, target trial emulation, surrogate index / long-term-effect prediction |
| `i-ops-analytics-and-metrics.md` | I. Product ops analytics & metrics | 20 | North Star + inputs, funnel/cohort, segmentation, Goodhart's law, regression to the mean, theory of constraints, leading vs lagging, root-cause, marketing mix modeling / incrementality, capital efficiency (Burn Multiple / Rule of 40 / Magic Number) |
| `j-strategy-frameworks.md` | J. Product / business strategy | 22 | Wardley Mapping, Playing to Win, Rumelt kernel, 7 Powers, OKR, scenario planning, real options, game theory, zero/positive-sum, signaling, credible threats, first-mover vs fast-follower, SWOT→TOWS, strategy tables, effectuation |
| `k-ai-augmented-2026.md` | K. AI-augmented decisioning (2026) | 12 | LLM as hypothesis generator/red-teamer, human-in-the-loop gates, sycophancy guards, decision agents, AI governance (NIST AI RMF / EU AI Act / ISO 42001), multi-agent debate & its limits, eval-driven shipping gates & agentic ROI |

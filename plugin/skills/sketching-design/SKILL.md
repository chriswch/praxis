...
- A **design sketch** with change map, pattern match, researched modern-practice baseline, proposed direction (including the data/contract shape and the reversal cost — step 5), and the first test to write (named with its test layer). Include a **Derived ACs / Assumptions** section when the spec arrived without acceptance criteria (Input preflight branch 2). → `Status: sketch`.
...
5. **Propose a direction.**
   ...
   - **Name the data and contract shape.** Whenever the story introduces or changes a domain type, a persisted shape, or a public payload, write it out at field level: the fields, which of them are genuinely optional, the invariants that always hold, and one line on what the shape makes *impossible*. Optionality is the decision that ages worst — a field left optional because the loader is lazy, rather than because the domain permits absence, spreads a null check into every consumer downstream. This is the highest-leverage thing a sketch says: `code-reviewing`'s first layer judges the same decision, but by then the tests encode it.
   - **Name the reversal cost.** Two sentences. Which single decision here would be most expensive to undo once the tests are written, and what would have to be true for the alternative to win instead. This is not a candidate list — you still commit to one direction — it is the line the design gate reads before approving.

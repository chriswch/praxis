...
2. **Holistic acceptance check — execute and evidence each AC.**
   ...
   - **Read the spec's Not Covered section before calling anything a gap.** An entry there is a recorded decision — a behavior the test posture deliberately left uncovered, already queued for the user at the ship gate — not missing work, and re-reporting it as a Gap would route the story to Rework over a choice that was already made. Verify the ACs that exist. The one time you overrule it: executing the ACs shows a Not Covered entry is genuinely consequential after all (it hides a security hole, a data-corruption path, a silent failure). Then promote it to a Gap and say what you observed that changed the assessment.

...
```bash
python3 -m praxis.runtime.run_state update-run-from-stage-result \
  --repo-root . \
...
`src/praxis/runtime/run_state.py` clears `run.routing.boundary_handoff_path` once
...
```bash
python3 -m praxis.runtime.story_boundary initialize-story-queue \
  --repo-root . \
...
```bash
python3 -m praxis.runtime.story_boundary pause-autopilot-for-stage-result \
  --repo-root . \
  --stage-result-path <artifact-dir>/results/<stage>.json \
  --timestamp <iso-8601-utc>
```
...
```bash
python3 -m praxis.runtime.story_boundary checkpoint-story-boundary \
  --repo-root . \
...
```bash
python3 -m praxis.runtime.story_boundary activate-next-story-from-boundary \
  --repo-root . \
  --timestamp <iso-8601-utc>
```
...
```bash
python3 -m praxis.runtime.story_boundary resume-story-run-from-disk \
  --repo-root . \
  --timestamp <iso-8601-utc>
```

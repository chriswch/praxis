...
**Pipeline**: `clarify-intent` → `design-sketch` → `tdd-loop` → **`verify-and-adapt`** → next slice (back to `clarify-intent`) or done.

## Input

- **Story-Level Behavioral Spec** (from `clarify-intent`) — required. The source of truth for what was supposed to be built.
- **AC Checklist** (from `tdd-loop`) — required. Shows completion status of each acceptance criterion.
- **Feedback Log** (from `tdd-loop`) — required, even if empty. Shows discoveries made during implementation.
- **Session Summary** (from `tdd-loop`) — required for medium+ tasks. Shows design decisions and spec feedback.
- **Design Sketch** (from `design-sketch`) — optional. May have been skipped or discarded during TDD.
- **Slice Map** (from `agile-story-slicer`) — optional. Only exists for multi-slice features.

## Workflow

1. **Validate inputs and triage.**
   - Gather: behavioral spec, AC checklist, feedback log, session summary (if medium+). If any required input is missing or TDD is incomplete (AC checklist has pending items, suite is not green), redirect back to `tdd-loop` to finish first.
   - Scale ceremony to task size:
     ...
     - **Large**: You shouldn't be here — should have been sliced. Redirect to `agile-story-slicer`.
...
3. **Reconcile spec vs. reality.**
   ...
   - Pull from tdd-loop's feedback log — discoveries already captured there flow into spec updates here.
...
4. **Capture emerged design knowledge.**
   - Pull design decisions from tdd-loop's session summary. Note anything that matters for future slices:
...
5. **Assess downstream impact (multi-slice only).**
   ...
   - Flag affected slices with a brief note. Do NOT re-plan or re-spec them — that's `clarify-intent`'s job when the slice is picked up. Just note the impact so the next cycle starts informed.
...
- **Verify behavior, not code.** Check "does this do what the spec said?" not "is this code clean?" Code quality is tdd-loop's refactor step and linus-style-review's job.
...
- **Don't add tests here.** If gaps are found, route back to tdd-loop. This step verifies; it doesn't implement.
- **Proportional ceremony.** A 20-minute TDD session doesn't need a 30-minute verification. Scale with complexity.
- **No gold-plating disguised as verification.** "We should also add logging" is a new requirement, not a verification finding. Route it through clarify-intent.

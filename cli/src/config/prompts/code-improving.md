...
skill via the Skill tool against that artifact. The skill walks the
driving-tdd commit range with `git diff {{baselineSha}}..HEAD` and
`git log {{baselineSha}}..HEAD`, auto-fixes Critical/High/Medium severity
findings, and never modifies test files.
...
The skill inspects the driving-tdd commit range with
`git diff {{baselineSha}}..HEAD` and `git log {{baselineSha}}..HEAD`,
auto-fixes Critical/High/Medium severity findings, and never modifies test
files.
Your final assistant message must be an improvement summary listing fixes
...
Risk note: like the driving-tdd stage, `bypassPermissions` runs against

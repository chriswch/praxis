...
function buildSelectionReason(candidate: RankedCandidate): string {
  const dependencyPart =
    candidate.dependsOnFindingIds.length > 0
...
function shouldGroupCandidate(candidate: RankedCandidate, group: RankedCandidate[]): boolean {
  ...
  return (
    group.some((entry) => overlapsAffectedPath(entry.finding, candidate.finding)) ||
...
function groupRelatedCandidates(
...
export function planRemediation(input: PlannerInput): {
  ...
  const assessmentDrivenCandidates = buildAssessmentDrivenCandidates(input.latestAssessment).filter(
    (finding) => isAtOrAboveSeverity(finding.severity, input.severityThreshold),
  );
  ...
  const slices: RemediationSliceRecord[] = selectedGroups.map((group, index) => {
    ...
    const scope = Array.from(
      new Set(group.flatMap((candidate) => candidate.finding.affected_paths)),
    );
  ...
  const selectedByFindingId = new Map<
    string,
    { priority: number; risk: RiskLevel; reason: string; groupedIn: string }
  >();
  ...
  if (slices.length === 0) {
    ...
    for (const slice of slices) {
      const selectedMeta = remediationMap.selection.selected.filter((item) =>
        slice.finding_ids.includes(item.finding_id),
      );

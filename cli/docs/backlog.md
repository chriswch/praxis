...
- [x] Stage 3 `auto-commit`: `permissionMode: "default"`, allowlist `[Bash]`, `timeoutMs: 300_000`. Skip with notice if `git status --porcelain` is empty. (§5.4) — S-005 (execution wired and verbatim `03-commit.txt`; `git status --porcelain` skip-with-notice still pending — no current test fails on this)
- [ ] After auto-commit message generation, harness runs `git add -A` and `git commit -m <finalText>` directly (not via the agent). Write SHA + message to `03-commit.txt`. (§5.4) — S-005 wires the `Deps.commit` hand-off (no-op stub prints stderr notice); real `git add -A` + `git commit -m` body and SHA capture land in S-006

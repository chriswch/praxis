...
- Artifact contracts — file paths, H2 schemas for `01-clarify-assess.md` and `04-code-review.md`, the SHA-prefix shape of `07-commit.txt`, the verbatim-finalText rule for `02-sketching-design.md` / `03-driving-tdd.md` / `05-code-improve.md` / `06-verifying-and-adapting.md`.
...
- Commit invariants — the SHA written to `07-commit.txt` matches `git log -1 --pretty=%H` and `state.stages["auto-commit"].commitSha`; `.gitignore` ends with a single `.praxis/` line (idempotent).

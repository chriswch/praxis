...
3. **`implement`** — full-tools execution against the working tree. Writes the changes the plan describes.
4. **`code-reviewing`** — read-only quality review of the uncommitted implement-stage changes via the `praxis:code-reviewing` skill. Emits a `## Decision` (`proceed` or `skip-improve`) that gates stage 5.
...
6. **`auto-commit`** — generates a Conventional-Commits message and runs `git add -A && git commit`. The harness performs the commit directly (not via the agent), captures the new SHA, and prepends it onto `06-commit.txt`. A clean working tree skips this stage entirely.
...
> **Plugin required.** Stages 2, 4, and 5 invoke skills from the `praxis` Claude Code plugin (`praxis:sketching-design`, `praxis:code-reviewing`, and `praxis:code-improving`). Install via `/plugin install praxis@<marketplace>` (see `.claude-plugin/marketplace.json` in the repo root). Plugin presence is not pre-flighted — a missing plugin surfaces as a `code-reviewing` validator failure (the agent emits "skill not found" in its final text; the harness flags the schema violation as a normal validator failure).
...
- [ ] `.praxis/runs/<run-id>/03-implement-log.md` is the agent's verbatim implement-stage finalText.
...
- [ ] The reporter printed: `[0/6 intent] captured → 00-intent.txt`, `[1/6 clarify-assess] starting…`, `[2/6 sketching-design] starting…`, `[3/6 implement] starting…`, `[4/6 code-reviewing] starting…`, `[5/6 code-improving] starting…` (or the skip line `[5/6 code-improving] skipped (skip-improve)`), `[6/6 auto-commit] starting…`, and `[run <run-id>] done — commit <sha>, <tokens> tokens, $<usd>`.
...
- **Clean-tree skip:** Run `--no-pause` against a repo where the implement stage produces no changes (e.g. an intent like "list the files in src/ and explain each"). Expect: stages `code-reviewing`, `code-improving`, and `auto-commit` all `completed`/`stopReason: "skipped"`, no `04-code-review.md`, `05-code-improve.md`, or `06-commit.txt`, no new commit. (`sketching-design` still runs — it is read-only and not part of the clean-tree skip set.)
...
- **SIGINT during implement:** Start `praxis run --no-pause "<long intent>"`, Ctrl-C while implement is mid-stream. Expect: `state.stages["implement"].status === "cancelled"`, `stopReason: "sigint"`, partial `03-implement-log.md` written, downstream stages not executed.
